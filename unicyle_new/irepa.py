...
def training(runs:int=20):
    print(f"Starting {runs} Irepa runs for Feedforward Network.......\n")

    # Irepa 1: 
    

    net = FeedForwardNet(fc1_dims=64, fc2_dims=64)
    print(net)
    eps = 0.
    for i in tqdm(range(runs)):
        ...
        init_points  = points(r=[2.-eps,1.25-eps, 0.5-eps], n = [50, 50, 50])

        states, values = statesValues(init_positions=init_points, horizon=100, terminal_model=terminal_model_unicycle, precision=1e-7)

        net = _training(net=net, xtrain=states, ytrain=values)

        torch.save(net, './nets3/net'+str(i)+'.pth')
        eps += 0.1
    print("Done")
